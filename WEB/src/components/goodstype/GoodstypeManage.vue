...
<template>
  <div>
      ...
      <div style="margin: 6px 0px 12px 0px;">
          ...
          -->
          <el-input class="input_name" v-model="name" placeholder="请输入物品分类名称" @keyup.enter.native="loadPost"
...
export default {
  ...
  methods: {
      ...
      dels(){
          ...
          // console.log(ids);
          this.$axios.get(this.$httpUrl + '/goodstype/deletes?ids=' + ids)
          .then(res => res.data).then(res => {        // 就是当.then()前的方法执行完后再执行then()内部的程序
              if (res.code == 200) {
                  this.$message({         // 雅观弹窗
                      message: '操作成功',
...
[aria-hidden] :focus-visible {
  ...
  outline: auto !important; /* 使用默认的轮廓线 */
